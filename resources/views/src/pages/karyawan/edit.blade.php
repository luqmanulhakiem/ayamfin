        ...
        <div class="col-12 col-md-6">
            <div class="card mb-4">
                ...
                <div class="card-body p-3 pt-0 mt-3 pb-2">
                    <form action="{{ route('karyawan.update', ['karyawan' => $karyawan]) }}" method="POST" id="categoryForm">
                        @csrf
                        <div class="form-group">
                            <label for="name">Nama Lengkap<span class="text-danger">*</span> </label>
                            <input id="name" name="full_name" type="text" class="form-control"
                                placeholder="Nama Lengkap Karyawan" value="{{ $karyawan->full_name }}" required>
                        </div>
                        <div class="form-group">
                            <label for="name">Posisi/Jabatan</label>
                            <input id="name" name="position" type="text" class="form-control"
                                placeholder="Cth: Tenaga Lapangan" value="{{ $karyawan->position }}">
                        </div>
                        <div class="form-group">
                            <label for="name">Gaji Pokok<span class="text-danger">*</span></label>
                            <div class="input-group">
                                <div class="input-group-text">Rp.</div>
                                <input value="{{ $karyawan->salary }}" id="name" name="salary" type="number"
                                    class="form-control" placeholder="Masukkan gaji karyawan" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="name">Nomer HP</label>
                            <input id="name" name="phone_number" type="number" class="form-control"
                                placeholder="Cth: 628XXZZZZYYYY" value="{{ $karyawan->phone_number }}">
                        </div>
                        <div class="form-group">
                            <label for="name">Alamat</label>
                            <textarea id="name" name="address" type="text" class="form-control" placeholder="Cth: Jln Suka Makmur">{{ $karyawan->address }}</textarea>
                        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function togglePengeluaranTypeForm() {
                if (typeSelect.value === 'pengeluaran') {
                    const dataFromBackend = "{{ $karyawan->type_pengeluaran ?? '' }}";
